...
1. **Tracked Workouts + Preferences**
   - Add `user_preferences` (or profile JSON) for `tracked_patterns` + `preferred_units`.
   - Add a "Tracked Workouts" section to Analytics and a simple preferences editor.
2. **Verification**
   - Run `scripts/verify_pr_logic.ts` and confirm tracked PR sections render as expected.
