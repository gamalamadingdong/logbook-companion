...
GitHub **Mobile Native Checks** run `35936264002` passed web checks, the Android debug build, the iOS simulator build, and exact trust-key checks on both native platforms: https://github.com/gamalamadingdong/logbook-companion/actions/runs/35936264002.

The dedicated Vercel project is `logbook-companion-updates`, connected to `gamalamadingdong/logbook-companion` with root `updates` and production branch `staging`. `updates.logbook.readyall.org` is live. The private key is provisioned as sensitive production-only `OTA_PRIVATE_KEY_BASE64`; the local ignored `.capgo_key_v2` remains mode 600 and must never be printed or committed. Preview signing keys are created only for a release branch and removed after merge.
...
PR #232 merged explicit native Diagnostics actions: manually trigger the native update check, reconcile downloaded verified bundles from `CapacitorUpdater.list()`, and install the newest ready bundle immediately through `CapacitorUpdater.set()` only when no PM5 capture is active. It removes the background async race and keeps source-controlled production halted. Native CI run `36031346569` passed, then archive/upload runs `36032256338` / `36032845458` delivered replacement TestFlight build `1790269749` from staging commit `d4ff0df3182b3131820f71e5d774a17613104d58`.

The replacement shell exposed two server-contract facts through Diagnostics. TestFlight reports `is_prod: false`, so beta.2 was correctly but undesirably blocked by `development_build_not_allowed`; beta.3 enabled development-classified beta clients while retaining emulator/app/native/platform/signature/replay gates. The plugin also reports the built-in bundle as `version_name: "1.0"`, not `"builtin"`, so beta.3 was blocked by `malformed_current_version`. PR #235 fixed this narrowly: an exact two-component `version_name === version_build` is treated as built-in, while normal OTA semver still uses replay/downgrade comparison.

Operator-confirmed first successful OTA: signed beta `1.0.0-beta.4` (sequence 4, merge `1bca1bafa608a751bf3bcba41d364637362a916c`) downloaded and installed from Diagnostics on TestFlight shell `1790269749`. The live endpoint now returns `up_to_date` and no URL for current beta.4. Temporary beta.4 preview signing material was removed; only the sensitive production key remains.

Resume order: (1) copy a post-activation Diagnostics report to preserve exact bundle/build/event evidence; (2) measure Training Block and other slow views with the new auth/network timing codes; (3) prove incompatible and tampered rejection on-device where practical; (4) publish an intentionally unready canary to prove automatic boot-failure rollback; (5) publish a higher known-good corrective bundle; (6) only then consider any automatic beta activation policy.

### Mobile performance and diagnostics investigation

The installed app exposed two usability gaps: no visible updater/build state, and Training Block taking up to roughly 30 seconds. Diagnostics and the first successful OTA are now proven; Training Block timing and remaining network/auth bottlenecks still require device evidence.
