...
PRs #156–#158 merged the `CompletedWorkoutV2` interval contract, three named synthetic fixtures, mapper-generated validator JSON, and Sam-reported strict Online Validator success. The optional validator suggestions (overall stroke rate, stroke count, drag factor) remain absent because the fixtures contain no measured values. A local development publishing branch now binds a selected fixture to a new owned LC test row, stores an independent service-only snapshot, and extends the existing fenced claim to exact interval payloads. Local build, Deno check, 477 Vitest tests, lint and disposable PostgreSQL checks passed; no new migration, Edge Function or UI has been deployed and no interval result has been POSTed yet. Deploy migration before Edge and staging UI, then explicitly publish and import each fixture. Production publishing remains disabled. See [interval rollout](../docs/concept2-mobile/development-interval-rollout.md).
