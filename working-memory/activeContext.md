...
The dedicated Vercel project is `logbook-companion-updates`, connected to `gamalamadingdong/logbook-companion` with root `updates` and production branch `staging`. `updates.logbook.readyall.org` is live. The private key is provisioned as sensitive production-only `OTA_PRIVATE_KEY_BASE64`; the local ignored `.capgo_key_v2` remains mode 600 and must never be printed or committed. The production endpoint was directly verified over TLS to return `200`, `Cache-Control: private, no-store`, `kind: blocked`, `error: channel_halted`, and no URL for both compatible and incompatible requests.
