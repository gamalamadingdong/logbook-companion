...
**Resume at [Concept2 publishing handoff](../docs/concept2-mobile/resume-publishing.md).** Current checks include 457 Vitest tests, disposable PostgreSQL tests, Deno check, staging and production builds, staging bundle isolation scan, and lint with no errors. Four targeted migrations are live and recorded; both new RPCs are service-only. The first fixed-distance development write/read round trip is proven. Next: change one variable by deliberately expiring the development access token, refresh through the UI, then save and publish a second distinct fixed-distance row and verify the same exact-ID import behavior. Production publishing and mobile remain deferred.
