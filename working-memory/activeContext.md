...
**Resume at [Concept2 publishing handoff](../docs/concept2-mobile/resume-publishing.md).** Current checks include 457 Vitest tests, disposable PostgreSQL tests, Deno check, staging and production builds, staging bundle isolation scan, and lint with no errors. Four targeted migrations are live and recorded; both new RPCs are service-only. The first fixed-distance development write/read round trip and real expired-token refresh are proven. Next: save and publish a second distinct fixed-distance row with the refreshed credential, then repeat the exact-ID import check. Use the [official API snapshot](../docs/concept2-mobile/concept2-logbook-api-reference.md) to plan broader result-shape support after repeatability is proven. Production publishing and mobile remain deferred.
