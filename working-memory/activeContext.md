...
## Local branch — manual interval publication and plan selection (not deployed)

Branch `feature/manual-interval-concept2` in `.worktrees/manual-result-c2-bridge` adds searchable saved templates or pasted RWN as optional plan sources in the general completion form. A chosen template saves `workout_logs.template_id`, its name, and an RWN snapshot in the versioned result; planned targets never become measured values. Measured interval cards carry distance/time interval kind, work/rest distance and time. Only complete Concept2 RowErg results with reconciled full interval detail reach the explicit development publish action; runs and other machines remain LC-only. The server adapter maps them into the existing `CompletedWorkoutV2` and the new migration independently checks the exact provider payload at claim. Local full Vitest suite, disposable PostgreSQL suite, Deno check, staging build, lint of changed implementation, and staging secret scan passed. The migration and Edge Function have **not** been applied/deployed live, the staging UI has **not** been updated, and no real manually entered interval result has been published. Controlled live 409/422 checks and Concept2's requirements reply remain pending. Keep production publishing disabled and legacy `publish-to-c2` untouched.
