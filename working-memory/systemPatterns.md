...
-   **Data model (unified)**: Single `athletes` table + `team_athletes` junction (with optional `squad` TEXT column). Coaching tables (`coaching_sessions`, `coaching_session_crews`, `coaching_session_crew_positions`, `coaching_athlete_notes`, `coaching_erg_scores`, `coaching_boatings`) are team-scoped via `team_id` column.
...
-   **Context hook**: `src/contexts/CoachingContext.tsx` — React Context provider wrapping all coaching routes. Exposes `userId`, `teamId`, `teamName`, `teamRole: TeamRole | null`, `orgId: string | null`, `activeTeam: UserTeamInfo | null`, `teams`, `teamsByOrg`, `isLoadingTeam`, `teamError`, `hasTeam`, `switchTeam()`, `refreshTeam()`. Old `src/hooks/useCoachingContext.ts` is a re-export shim. localStorage key is user-scoped (`lc_selected_team_${userId}`). Single provider instance — team switching immediately updates all consumers.
-   **Error handling pattern**: All coaching UI handlers wrap mutations in try/catch with `toast.error()` (Sonner). Service calls guarded by `if (!teamId) return;` where `teamId` is required. Org-wide queries use `orgId` from context, never derive locally.
...
-   **Schedule vs Boatings boundary**: `CoachingSchedule` is the primary daily-report workflow. `CoachingBoatings` is secondary templates/history tooling for reusable crew records, shell history, and session-linked references.
