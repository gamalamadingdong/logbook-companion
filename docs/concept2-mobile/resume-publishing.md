...
Local branch `feature/concept2-publication-core-fixed-time` adds the first shared-core slice: versioned `CompletedWorkoutV1`, server-only manual row normalization, pure fixed-distance/fixed-time payload mapping, mapper-versioned durable payload snapshots and a thin fixed-time development selector. Fixed distance retains its proven payload. Fixed time passes strict Online Validator checking as `FixedTimeSplits` for 30:00 / 7,321 m. Local verification now has 463 Vitest tests plus the disposable PostgreSQL and Deno/build/bundle gates. This work is not live: migration/function/UI are undeployed and no fixed-time development POST/read-back/re-import has occurred.
