...
The hardware-proven baseline is `PM5CompletedCaptureV1`. Published `@readyall/erglink@0.6.0` adds backward-readable capture v2, deterministic validation, shared IndexedDB/SQLite stores, and interrupted-upload recovery. The current E4 branch binds immutable owner/programming context to the local envelope, saves before network use, retries only for the matching athlete, ingests idempotently through existing `workout_logs.external_id` uniqueness and owner RLS, surfaces summary/retry state, and acknowledges only after LC returns its workout UUID. Automated tests make no live write.
...
| PM5 capture envelope | Stable v2 ID/version, raw evidence, deduplicated/time-aligned strokes, splits, three summaries, start/verification evidence, lifecycle, validator, shared browser/mobile stores, and owner-bound ingestion are implemented. Capture-v1 summary semantics are hardware-proven; v2/E4 are fixture-proven. | Run a controlled authenticated live smoke, then complete physical installed-device proof. |
...
| Detailed result | Capture v2, validation, projection, and owner-bound local-first ingestion preserve searchable totals plus root-compatible detail and retained raw evidence. | Run a controlled authenticated live smoke, then prove projected payloads through the development provider. |
...
4. **LC capture ingestion (E4):** implemented in the current branch with fixture proof; controlled authenticated live smoke remains.
