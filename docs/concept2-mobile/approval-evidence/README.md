...
| Fixed-distance intervals + measured rest meters | Passed locally for fixed-distance and fixed-time classification | Pending strict check of corrected payload | Pending after coordinated rollout | Pending | Historical result `86938` was published as `VariableInterval`; it does not count as fixed-shape provider evidence. |
