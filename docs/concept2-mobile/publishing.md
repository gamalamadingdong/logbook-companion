...
**Current status:** Fixed-distance and fixed-time summaries, three synthetic interval classes, two saved general manual RowErg summaries, and three manually entered interval results are proven in Concept2 development with exact-ID readback/import. Sam confirmed provider-side synthetic interval breakdowns and supplied the mixed manual result page, which displays Variable Interval, 1,200 m / 5:00 work, 1:00 rest, 6:00 overall, and the two correct interval rows. The saved manual results are development IDs `86932` (7,500 m), `86933` (10,000 m), `86935` (1,000 m / 240 s work, fixed-distance intervals), `86936` (1,200 m / 300 s work, mixed intervals), and `86937` (2,250 m / 540 s work, fixed-time intervals). Live verification found one publication attempt and one imported row for each manually entered interval result. These tests do not prove real PM5 capture. The general manual result page now uses the shared core. PRs #179–#182 add source-preserving import, incomplete-result guards, published-result edit protection, and cumulative work-plus-recovery volume semantics; targeted staging smoke remains.

## Manual interval bridge in staging (2026-09-18)

The general LC completion flow now separates plan from result: searchable saved LC templates and pasted RWN set only planned targets, while manually entered work/rest measurements form the durable result. The optional template link and RWN snapshot stay with the LC row. Complete Concept2 RowErg distance, time, and variable intervals normalize server-side into `CompletedWorkoutV2` and use the existing mapper, publication fence, immutable payload snapshot, and exact-ID read-back. A new service-only SQL claim checks a payload reconstructed from the saved measurements. Local mapping, disposable PostgreSQL, full Vitest, Deno, lint, and staging build checks pass. Migration `20260918160118` and development Edge Function version 9 are live, with JWT verification on; the Vercel staging merge deployment passed. The first manually entered fixed-distance interval result, LC workout `4f037c45-e792-4896-9b90-bcbbb4c4e997`, became result `86935` with one dispatch, mapper version 2, and a `FixedDistanceInterval` payload. The mixed manual interval result, LC workout `f4704481-8523-48de-bdbf-097634b3745f`, became result `86936` with one dispatch, mapper version 2, and an ordered `VariableInterval` payload. Concept2 also displays 66 calories and 179 average watts; these were not in the LC publication payload and are not evidence of measured power or calorie capture. Its unusual generated page title is not the LC row name or planned RWN; the payload has no title field. After correcting an omitted interval type on the same LC row, Sam published manually entered fixed-time intervals as result `86937`; exact-ID readback succeeded. Live state shows one dispatch, mapper version 2, and one imported row. Provider-page inspection and template selection remain to be checked. Form validation for missing interval types merged in PR #173. Production publishing remains disabled, and ErgLink capture/telemetry proof is a separate track.

## Fixed interval rest distance correction (2026-09-18)

A saved manual 2 × 500 m RowErg result with 1 m measured during rest was published to Concept2 development as result `86938`. The previous mapper and SQL publication claim classified any nonzero rest distance as `VariableInterval`, even though both work intervals were fixed at 500 m. The immutable publication snapshot shows one dispatch, mapper version 2 and an exact result ID. Keep that historical result as published; do not retry it or rewrite its snapshot.

[Concept2's API examples](https://log.concept2.com/developers/documentation/) allow a workout-level `rest_distance` on `FixedDistanceInterval`. The corrected shared mapper and service-only SQL claim choose fixed distance when all measured work intervals have the same distance, or fixed time when all measured work intervals have the same duration, regardless of measured rest distance. They preserve the rest total at workout level and omit per-interval `rest_distance` for fixed shapes. The local regression cases cover both fixed shapes with measured rest distance; the previously proven mixed variable shape remains variable. Migration `20260918192002` is recorded live and `concept2-development-auth` version 10 is deployed with JWT verification on. Read-only reconstruction of the saved result now yields `FixedDistanceInterval` with 1,000 m work and 1 m rest distance; result `86938` retains its original immutable `VariableInterval` snapshot and one attempt. The live helper remains service-role only, and an unauthenticated Edge request returned 401. A strict Online Validator check and a new development write/read-back for fixed intervals with nonzero rest distance remain pending. Production publishing stays disabled.

## Completed-result integrity checkpoint (2026-09-19)

PRs #179–#182 are merged into staging. Concept2 import now preserves LC manual/ErgLink source evidence, partial manual detail no longer becomes a claimed complete total, imported provider detail survives normalization, and published or uncertain general manual development results are protected from result-bearing edits. Sam reports applying migration `20260918210000_guard_published_manual_results.sql`. Cumulative training volume is measured work plus measured recovery, while RWN, performance metrics and Concept2 `distance` remain work-based.

Resume with the ordered checks in the [capture/storage audit](capture-storage-audit.md): targeted manual staging smoke, exact-ID and near-match import identity, explicit provider association/revision behavior, then the stable ErgLink capture envelope and real PM5 evidence.
