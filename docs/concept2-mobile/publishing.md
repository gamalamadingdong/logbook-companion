...
**Current status:** Fixed-distance and fixed-time summaries, three synthetic interval classes, two saved general manual RowErg summaries, and two manually entered interval results are proven in Concept2 development with exact-ID readback/import. Sam confirmed provider-side synthetic interval breakdowns and reported a successful check of the manual interval. The saved manual results are development IDs `86932` (7,500 m), `86933` (10,000 m), `86935` (1,000 m / 240 s work, fixed-distance intervals), and `86936` (1,200 m / 300 s work, mixed intervals). Live verification found one publication attempt and one imported row for each manually entered interval result. These tests do not prove real PM5 capture. The general manual result page now uses the shared core; functional testing continues.

## Manual interval bridge in staging (2026-09-18)

The general LC completion flow now separates plan from result: searchable saved LC templates and pasted RWN set only planned targets, while manually entered work/rest measurements form the durable result. The optional template link and RWN snapshot stay with the LC row. Complete Concept2 RowErg distance, time, and variable intervals normalize server-side into `CompletedWorkoutV2` and use the existing mapper, publication fence, immutable payload snapshot, and exact-ID read-back. A new service-only SQL claim checks a payload reconstructed from the saved measurements. Local mapping, disposable PostgreSQL, full Vitest, Deno, lint, and staging build checks pass. Migration `20260918160118` and development Edge Function version 9 are live, with JWT verification on; the Vercel staging merge deployment passed. The first manually entered fixed-distance interval result, LC workout `4f037c45-e792-4896-9b90-bcbbb4c4e997`, became result `86935` with one dispatch, mapper version 2, and a `FixedDistanceInterval` payload. The mixed manual interval result, LC workout `f4704481-8523-48de-bdbf-097634b3745f`, became result `86936` with one dispatch, mapper version 2, and an ordered `VariableInterval` payload. Fixed-time manual intervals and template selection remain to be checked. Production publishing remains disabled, and ErgLink capture/telemetry proof is a separate track.
