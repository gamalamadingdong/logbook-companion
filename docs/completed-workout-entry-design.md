...
## Follow-on staging slice: template choice and manual interval results (local, 2026-09-18)
...
Only a fully measured and reconciled Concept2 RowErg interval result can show the separate development publish step. The preview shows work totals and rest totals. Server normalization and the database claim independently reject partial detail, missing interval kind, mismatched totals, altered payloads, wrong ownership, and repeated dispatch. Other activities and machines remain useful LC records. This slice is local until the shared backend migration, Edge Function, and staging UI are rolled out in order; it does not enable production publishing or prove PM5 capture.
