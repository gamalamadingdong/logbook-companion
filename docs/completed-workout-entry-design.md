...
## Follow-on staging slice: template choice and manual interval results (2026-09-18)

The manual form now offers a search across saved LC template names, canonical names, and RWN, alongside pasted RWN. Selecting a template fills planned targets, retains its ID/name plus a copy of its RWN on the result, and indexes the template ID for LC comparison. Editing the RWN clears the link. Neither path fills actual measurements. Quick result fields remain above the optional plan controls; measured interval cards follow the plan. Work cards ask whether the completed interval was distance or time based, and the person enters actual distance and work time. Rest cards hold actual rest time and moving distance.

For a workout started blank, the end-state UX should suggest likely templates after the measured result is available, then let the person confirm a link. Reuse LC’s exact canonical matching/scoring where the measured structure supports it. Do not call the current `matchWorkoutToTemplate` mutation on this new result directly: it changes only `workout_logs.template_id`, leaving the versioned result snapshot out of sync with the publication claim. Candidate suggestions and an atomic explicit link are a follow-on UX slice.

Only a fully measured and reconciled Concept2 RowErg interval result can show the separate development publish step. The preview shows work totals and rest totals. Server normalization and the database claim independently reject partial detail, missing interval kind, mismatched totals, altered payloads, wrong ownership, and repeated dispatch. Other activities and machines remain useful LC records. The shared backend migration, Edge Function, and staging merge deployment are complete. Sam published and checked the first manually entered fixed-distance interval result (`86935`); live state shows one dispatch and one imported result linked to the exact LC workout ID. Sam also published a mixed distance/time interval result (`86936`); live verification found one dispatch, one imported result, and the ordered work/rest detail. Sam corrected an omitted type in the saved fixed-time interval row and published it as result `86937`; exact-ID readback and one imported row are verified. Form validation for this omission is in PR #173, pending staging merge. Provider-page inspection and template selection remain to be checked. This does not enable production publishing or prove PM5 capture.
