-- Development-only interval fixture snapshots. Client-owned workout rows are not trusted
-- as publication authority; this table and both RPCs are service-role only.
create table public.c2_development_fixture_workouts (
 workout_id uuid primary key references public.workout_logs(id) on delete restrict,
 user_id uuid not null references auth.users(id) on delete cascade,
 fixture_name text not null check (fixture_name in
...
alter table public.c2_development_fixture_workouts enable row level security;
revoke all on public.c2_development_fixture_workouts from public,anon,authenticated;
grant select,insert on public.c2_development_fixture_workouts to service_role;

create function public.c2_development_create_fixture_workout(
...
declare workout_id uuid; finished_at timestamptz; distance_value integer; duration_value numeric;
 shape text;
begin
 if p_fixture_name not in ('fixed_distance_intervals_2x500m','fixed_time_intervals_3x120s','variable_intervals_mixed')
...
declare r public.c2_development_auth; w public.workout_logs;
 ...
 fixture jsonb; fixture_name text; expected jsonb; interval_payload jsonb;
 ...
 if p_action='list' then
  ...
  if retry_existing then
   ...
   update public.c2_development_publications set status='outcome_unknown',
    ...
    mapper_version=case when fixture is null then 1 else 2 end,resolved_at=null,resolution_evidence=null,resolved_by=null where id=pub.id;
  else
   insert into public.c2_development_publications
    ...
    values(p_user_id,w.id,r.provider_user_id,'outcome_unknown',op,body,case when fixture is null then 1 else 2 end);
  end if;
